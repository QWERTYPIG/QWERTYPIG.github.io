...
### 解題過程
#### 變數宣告
{% highlight cpp linenos linenostart=3 %}
int n,mxfail=-1,mnpass=101;
vector<int>v;
{% endhighlight %}
n同題述，mxfail為最高不及格分數，mnpass為最低及格分數，v存分數
#### 輸入、排序
{% highlight cpp linenos linenostart=6 %}
cin>>n;
v.assign(n,0);
...
{% endhighlight %}
#### 輸出、決定數字
{% highlight cpp linenos linenostart=10 %}
for(int i:v){
    cout<<i<<" ";
...
接著交換行列座標(第18行)
#### 輸出
{% highlight cpp linenos linenostart=25 %}
    cout<<r<<" "<<c<<"\n";
    for(int i=0;i<r;i++){
...
### 解題過程
#### define
{% highlight cpp linenos linenostart=2 %}
#define F first
#define S second
{% endhighlight %}
我就懶
#### 變數宣告
{% highlight cpp linenos linenostart=5 %}
int n,ans=0;
deque<pair<int,int>>dq;
{% endhighlight %}
n同題述，ans存答案，dq同前述
#### 輸入、排序
{% highlight cpp linenos linenostart=8 %}
cin>>n;
dq.assign(n,{});
...
第9行初始化dq
#### 合併線段、計算答案
{% highlight cpp linenos linenostart=12 %}
while(dq.size()){
    if(dq.size()==1){
        ans+=dq[0].S-dq[0].F;
        dq.pop_front();
...
#### 輸出
{% highlight cpp linenos linenostart=29 %}
cout<<ans;
{% endhighlight %}
### 完整程式碼
```cpp
...
#### 輸入優化
{% highlight cpp linenos linenostart=16 %}
ios::sync_with_stdio(0);
cin.tie(0);
{% endhighlight %}
本題多筆測資且輸入多，不加會TLE
#### 輸入、初始化、存圖
{% highlight cpp linenos linenostart=18 %}
while(cin>>n){
...
#### 找樹直徑、輸出
{% highlight cpp linenos linenostart=26 %}
    fardot=0;
    dep[0]=1;
    dfs(0);
    int st=fardot;
